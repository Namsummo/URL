...
function displayProducts(productsToShow = products) {
    ...
    productGrid.innerHTML = productsToShow.map(product => `
        <div class="product-card" data-id="${product.id}" onclick="navigateToProductDetail(${product.id})">
        ...
        loginForm.addEventListener('submit', function(e) {
            ...
            if (user) {
                ...
                if (rememberMe) {
                    ...
                    sessionStorage.removeItem('clientUser'); // Xóa session nếu có
                } else {
                    sessionStorage.setItem('clientUser', JSON.stringify(userData));
                    localStorage.removeItem('clientUser'); // Xóa local storage nếu có
                }
...
function setActiveNavLink() {
    ...
    console.log("[setActiveNavLink] Current Raw Path:", currentPagePath);
    // Target the direct list items within .nav-links
    ...
    const normalizedCurrentPagePath = normalizePath(currentPagePath);
    console.log("[setActiveNavLink] Normalized Current Path:", normalizedCurrentPagePath);

    let activeLinkSet = false;

    navListItems.forEach(li => {
        // Find the direct anchor link within the li
        const link = li.querySelector(':scope > a'); // :scope ensures it's a direct child

        // Remove active class from the link if it exists
        if (link) {
            link.classList.remove('active');
        }

        // Skip if no direct link (like the user menu li) or if link is invalid
        ...
        try {
            ...
            console.log(`[setActiveNavLink] Comparing: Link Path '${linkPath}' === Current Path '${normalizedCurrentPagePath}' (Href: ${link.href})`);

            // Kiểm tra nếu đang ở trang chi tiết sản phẩm
            if (normalizedCurrentPagePath.includes('product-detail.html') && linkPath.includes('products.html')) {
                link.classList.add('active');
                console.log("[setActiveNavLink] >>> SET ACTIVE for product detail:", link.href);
                activeLinkSet = true;
            }
            // Kiểm tra các trường hợp thông thường
            else if (linkPath === normalizedCurrentPagePath) {
                link.classList.add('active');
                console.log("[setActiveNavLink] >>> SET ACTIVE:", link.href);
                activeLinkSet = true;
            }
        } catch (e) {
            console.error('[setActiveNavLink] Error processing link:', link.outerHTML, e);
        }
    });

    if (!activeLinkSet) {
        console.warn("[setActiveNavLink] >>> No active link set based on path comparison.");
    }
}

// Placeholder for updateAuthStatus if needed globally
function updateAuthStatus() {
    const currentUser = JSON.parse(localStorage.getItem('clientUser')) || 
                       JSON.parse(sessionStorage.getItem('clientUser'));
    const authLinkContainer = document.querySelector('.nav-links li:last-child');

    if (!authLinkContainer) {
        console.error("Auth link container not found");
        return;
    }

    // Clear previous dynamic content (dropdown menu)
    const existingDropdown = authLinkContainer.querySelector('.dropdown-menu');
    if (existingDropdown) existingDropdown.remove();
    const existingUserMenu = authLinkContainer.querySelector('.user-menu');
    if (existingUserMenu) existingUserMenu.remove();

    if (currentUser) {
        // Create user menu if it doesn't exist or update if it does
        ...
        dropdownMenu.innerHTML = `
            <li><a href="profile.html"><i class="fas fa-user-circle"></i> Tài khoản</a></li>
        ...
        // Replace the content of the container LI with the user menu
        authLinkContainer.innerHTML = ''; // Clear current content first
        authLinkContainer.appendChild(userMenu);
       
    } else {
        // Ensure login link is shown if no user
        authLinkContainer.innerHTML = '<a href="auth.html">Đăng Nhập</a>'; 
    }
}

// Make sure to call updateAuthStatus and setActiveNavLink in the correct order on DOMContentLoaded
document.addEventListener('DOMContentLoaded', function() {
    // First set the active nav link based on the current page
    setActiveNavLink();
    
    // Then update the auth status to show user info or login link
    ...
    // Call other page-specific initializations here...
    if (document.querySelector('.orders-section')) {
        displayOrders();
    }
    if (document.querySelector('.profile-section')) {
        // Initialization logic for profile page if needed beyond updateAuthStatus
    }
                ...
                if (file) {
                    ...
                    reader.onload = function(e) {
                        ...
                        if (currentUser) {
                            ...
                            if (sessionStorage.getItem('clientUser')) {
                                sessionStorage.setItem('clientUser', JSON.stringify(currentUser));
                            }
...
// Keep other functions like toggleChat, sendMessage etc. outside the DOMContentLoaded if they are needed globally
function toggleChat() { 
...
// Xử lý tìm kiếm trong navigation
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Nếu đang ở trang products.html và có từ khóa tìm kiếm
    if (window.location.pathname.includes('products.html')) {
        const savedSearchTerm = sessionStorage.getItem('searchTerm');
        if (savedSearchTerm) {
            // Điền từ khóa vào ô tìm kiếm của trang sản phẩm
            const productSearchInput = document.getElementById('searchInput');
            if (productSearchInput) {
                productSearchInput.value = savedSearchTerm;
                // Kích hoạt tìm kiếm
                searchProducts();
            }
            // Xóa từ khóa đã lưu
